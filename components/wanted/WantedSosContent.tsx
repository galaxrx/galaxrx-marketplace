...
import SosTooltip from "@/components/wanted/SosTooltip";
...
export default async function WantedSosContent({
  searchParams,
}: {
  ...
  return (
    <div className="w-full max-w-full p-0">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-heading font-bold text-gold inline-flex items-center">
          SOS — Urgent requests
          <SosTooltip />
        </h1>
      ...
      <div className="mb-4 flex flex-wrap gap-2 items-center">
        <Suspense fallback={<div className="flex-1 min-w-[12rem] max-w-xl h-10 rounded-lg bg-white/5 animate-pulse" />}>
          <WantedSearch basePath="/wanted/sos" />
        </Suspense>
        <Suspense fallback={<div className="h-9 w-48 rounded-lg bg-white/5 animate-pulse" />}>
          <WantedFilters basePath="/wanted/sos" />
        </Suspense>
      </div>
      {items.length === 0 ? (
        <div className="bg-mid-navy border border-[rgba(161,130,65,0.18)] rounded-xl p-8 text-center">
          <p className="text-white/70">No SOS requests right now.</p>
          <Link href="/wanted/browse" className="mt-3 inline-block text-gold font-medium hover:underline">Browse all wanted requests</Link>
        </div>
      ) : (
        <>
          ...
          {totalPages > 1 && (
            <div className="flex flex-wrap justify-center gap-2 mt-6">
              {clampedPage > 1 && (
                <Link
                  href={`/wanted/sos?${new URLSearchParams({ search, urgency, state, sort, page: String(clampedPage - 1) }).toString()}`}
