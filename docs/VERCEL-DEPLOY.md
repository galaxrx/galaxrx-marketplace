...
| Variable | Notes |
|----------|--------|
| `DATABASE_URL` | Production DB. On **Supabase + Vercel**, use a **pooler** URI (**Session** port **5432** or **Transaction** port **6543**). **Do not** use `db.<project>.supabase.co` from Vercel — it often returns **P1001** (IPv4). `prisma migrate deploy` uses this same URL. |
...
- First deploy: applies all migrations to the **production** database using **`DATABASE_URL`**. It must be a **reachable** string from Vercel (Supabase **pooler**, not `db.*.supabase.co`, unless you use Supabase **IPv4 add-on** for direct).
- Ensure **`DATABASE_URL`** is correct **before** the first deploy.
...
- [ ] Set `DATABASE_URL` to a **Supabase pooler** URI (reachable from Vercel)  
